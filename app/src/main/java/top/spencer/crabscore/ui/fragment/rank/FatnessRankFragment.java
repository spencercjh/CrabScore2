...
public class FatnessRankFragment extends BaseFragment implements MyRecycleListView, SwipeRefreshLayout.OnRefreshListener {
    ...
    private RankListAdapter fatnessRankListAdapter;
    private RankListPresenter rankListPresenter;
    private List<RankResult> fatnessRankList = new ArrayList<>(pageSize);
    ...
    public void setRecycleView() {
        fatnessRankListAdapter = new RankListAdapter(fatnessRankList, getContext());
        if (fatnessRankList.size() == 0) {
    ...
    public void showData(JSONObject successData) {
        pageNum++;
        boolean repeat = rankListPresenter.dealRankListJSON(successData.getJSONObject("result").getJSONArray("list"), fatnessRankList);
        if (repeat) {
            return;
        }
